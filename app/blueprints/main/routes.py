...
@main_bp.route("/setup-admin/<token>")
def setup_admin(token):
    """
    ⚠️ RUTA TEMPORAL: Crear usuario administrador.
    Eliminar esta ruta después de usarla por seguridad.
    """
    ...
    secret_token = os.getenv("SETUP_TOKEN")
    ...
    admin = User(
        first_name="Admin",
        last_name="Peluzza",
        email="admin@marroquineria.com",
        is_admin=True,
        loyalty_level="platinum",
        loyalty_points=10000
    )
    admin.set_password("Admin123!")  # ⚠️ CAMBIA ESTA CONTRASEÑA DESPUÉS
    ...
    return jsonify({
        ...
        "instrucciones": "Inicia sesión y cambia la contraseña inmediatamente"
    })